Website Script Scanner: Code Explanation
Overview
The Website Script Scanner is a Python script that helps you scan websites for JavaScript files, maintain an inventory, authorize scripts, and check the integrity of scripts. This tool is designed to help meet PCI DSS 4.0 requirement 6.4.3, which mandates managing payment page scripts to ensure their authorization, integrity, and necessity.

How It Works
The script performs the following main tasks:

Input and Validation of URLs
Fetching Scripts and Headers
Baseline Creation and Storage
Comparison of Current Data with Baseline
Generating HTML Reports
Displaying Results
Let's break down each part of the code to understand its functionality.
...
Imports: Various libraries are imported to handle HTTP requests (requests), parse HTML content (BeautifulSoup), compute hashes (hashlib), handle JSON data (json), work with file paths (os), manage dates (datetime), and parse URLs (urlparse).
Constants: BASELINE_FILE and REPORT_FILE are defined to store the baseline data and the HTML report, respectively.
2. Utility Functions
add_scheme_if_missing(url, base_url)
python
Copy code
def add_scheme_if_missing(url, base_url):
    if url.startswith('//'):
        ...
        return base_url + url
    return url
This function ensures that URLs have a scheme (http/https). If the URL starts with //, it prepends https:. If the URL lacks a scheme, it prepends the base_url.
...
def validate_url(url):
    try:
        ...
        return False
This function checks if a URL is properly formatted by using urlparse to parse it and then verifying that it contains both a scheme and a network location.

get_scripts_and_headers(url)
python
Copy code
...
This function fetches the scripts and headers from a given URL:

It sends a GET request to the URL.
It parses the response content with BeautifulSoup to find all <script> tags.
For each script with a src attribute, it constructs the full URL, fetches the script content, computes its SHA-256 hash, and stores this information.
It also extracts and returns the response headers.
has_csp(headers)
python
Copy code
def has_csp(headers):
    return 'Content-Security-Policy' in headers
This function checks if the Content-Security-Policy header is present in the headers.
...
This function saves the baseline data to a JSON file.
...
def load_baseline():
    ...
    return {}
This function loads the baseline data from a JSON file if it exists. If the file does not exist, it returns an empty dictionary.

4. Comparison Function
compare_data(baseline, current)
python
Copy code
...
This function compares the current data with the baseline to detect changes:

It creates dictionaries for baseline and current scripts with their URLs as keys and hashes as values.
It checks for new, changed, and removed scripts.
It also checks for changes in headers.
It returns a boolean indicating if changes were detected and a dictionary of the changes.
5. Authorization Function
prompt_authorization(script_info)
python
Copy code
...
This function prompts the user to authorize each script and provide a justification:

It iterates over the script information.
For each script, it asks the user if they authorize it.
If authorized, it prompts the user to provide a justification.
It returns a list of authorized scripts with their justifications.
6. Report Generation
...
def generate_html_report(websites, changes_summary, details, baseline_data, csp_data):
    ...
    # Write the generated HTML content to the report file
...
def display_results(changes_summary, details, baseline_data, csp_data):
    ...
    for url, scripts in baseline_data.items():
        ...
        for script in scripts['scripts']:
            ...
            print("\n")
This function displays the results of the scan in the console:

It prints a summary of changes detected, including new, changed, and removed scripts, CSP presence, and header changes.
It provides detailed information about each change.
It also displays the authorized scripts inventory and their justifications.
8. Main Function
main()
python
Copy code
...
if __name__ == '__main__':
    main()
The main function provides a menu-driven interface to the user:

It allows the user to input a list of websites.
It performs a baseline scan, saving authorized scripts and their justifications.
It scans for changes, comparing current scripts and headers with the baseline.
It generates an HTML report and displays the results in the console.
The user can exit the program by choosing option 4.
